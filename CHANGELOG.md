...
- **Sticky header viewport model stabilised across desktop, mobile, and installed web apps:** Removed the split fixed-vs-sticky header behaviour and moved the layout to one shared header-height system using `svh`/`dvh` sizing and safe-area-aware spacing. This avoids the viewport drift that reappeared after restoring desktop stickiness.
- **Mobile menu no longer renders partially offscreen:** The narrow right-edge sheet failure was caused by the mobile menu still behaving like a shrinkable slide-over panel. On mobile it now becomes a full-screen fixed layer with explicit viewport width/height bounds, border-box sizing, and safe-area padding so it cannot collapse outside the visible viewport.
