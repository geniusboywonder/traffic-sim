...
const SCENARIOS = {
  H: {
    ...
    dwellTime: 45,
    description:
      "TIA worst-case — 840 trips, sharp 45-min peak, no reductions applied",
  },
  M: {
    label: "Medium",
    trips: 650,
    ...
    ratRunThreshold: 0.8,
    dwellTime: 45,
    description: "Notional 15% reduction — 650 trips over 60-min window",
  },
  L: {
    label: "Low",
    trips: 500,
    peakMin: 75,
    ratRunThreshold: 0.85,
    dwellTime: 45,
    description: "Notional 40% reduction — 500 trips over 75-min window",
  },
};
...
const TRIP_ORIGINS = [
  ...
  {
    id: "dreyersdal_s",
    label: "Dreyersdal Rd (S)",
    pct: 0.13,
    type: "external",
  },
  { id: "christopher", label: "Christopher Rd", pct: 0.04, type: "local" },
  { id: "starke_n", label: "Starke Rd (N)", pct: 0.1, type: "local" },
...
const WHATIF_GROUPS = [
  {
    ...
    items: [
      {
        id: "parking_conflict",
        label: "On-street parking conflict",
        description:
          "22 bays on Ruskin Rd — loses one moving lane during drop-off",
        effect: "−35% Ruskin Rd capacity",
      },
      {
        id: "narrow_carriageway",
        label: "Narrow carriageway friction",
        description:
          "Class 5 road width never evaluated against 840 peak trips",
        effect: "×0.85 capacity on Ruskin Rd",
      },
      {
        id: "pedestrian_conflict",
        label: "Pedestrian-vehicle conflict",
        description: "No footpaths — learners walk on carriageway",
        effect: "×0.80 capacity friction",
      },
      {
        id: "aristea_delay",
        label: "Aristea Rd egress geometry",
        description: "Cul-de-sac conversion — no turning analysis done",
        effect: "+30s per vehicle at egress",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "stp",
        label: "School Travel Plan (STP)",
        description:
          "Conservative STP estimate — reduces total vehicle trips by 15%",
        effect: "−15% total trips",
      },
      {
        id: "staggered_bells",
        label: "Staggered bell times (15 min)",
        description:
          "Spreads arrival spike across two waves — softens peak sharply",
        effect: "Splits peak into 2 waves",
      },
      {
        id: "marshal",
        label: "Traffic marshal at Ruskin/Leyden",
        description:
          "Increases junction throughput by 25% — reduces ingress queue",
        effect: "+25% junction throughput",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "walkers",
        label: "Walkers & cyclists (15%)",
        description:
          "TIA applied zero modal reduction — contested by independent review",
        effect: "−15% vehicles (~126 fewer)",
      },
      {
        id: "public_transport",
        label: "Public transport — MBT / GABS (10%)",
        description:
          "Bus routes on Main Rd and Ladies Mile Rd entirely unacknowledged",
        effect: "−10% vehicles (~84 fewer)",
      },
      {
        id: "lift_clubs",
        label: "Lift clubs / carpooling (5%)",
        description: "No carpooling factor modelled in TIA",
        effect: "−5% vehicles (~42 fewer)",
      },
    ],
  },
];
...
function calcVC(segId, trips, whatIfActive, whatIfItems) {
  ...
  if (whatIfActive && segId === "ruskin") {
    ...
    if (whatIfItems.pedestrian_conflict) cap *= 0.8;
  }
  ...
  if (whatIfActive) {
    ...
    if (whatIfItems.public_transport) reduction *= 0.9;
...
function losFromVC(vc) {
  if (vc < 0.6) return { los: "A–B", color: "#22c55e", label: "Free flow" };
  if (vc < 0.75) return { los: "C", color: "#eab308", label: "Stable" };
  if (vc < 0.9)
    return { los: "D", color: "#f97316", label: "Approaching unstable" };
  if (vc < 1.0) return { los: "E", color: "#ef4444", label: "Unstable" };
...
function RoadMap({ scenario, whatIfActive, whatIfItems }) {
  ...
  const getSegStyle = (segId) => {
    ...
    return {
      stroke: color,
      strokeWidth:
        seg?.class === "arterial" ? 6 : seg?.class === "collector" ? 4 : 3,
      strokeDasharray: isModified ? "8 4" : "none",
      opacity: 0.92,
    };
  };
  ...
  return (
    ...
    >
      ...
      {[
        ...
        {
          id: "ruskin",
          text: "Ruskin Rd ⬅ INGRESS",
          pos: [235, 350],
          anchor: "start",
        },
        {
          id: "aristea",
          text: "Aristea Rd ⬆ EGRESS",
          pos: [368, 420],
          anchor: "start",
        },
        {
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
        {[
          ...
          [
            "Avg wait",
            wait > 0 ? `${Math.floor(wait / 60)}m ${wait % 60}s` : "—",
          ],
          ...
          <div key={k}>
            ...
            <div
              style={{
                color:
                  k === "Status" && v.includes("UNSTUDIED") ? "#a855f7" : color,
                fontSize: 10,
                fontWeight: 700,
                fontFamily: "monospace",
              }}
      ...
      {showDelta && whatIfActive && (
        <div
          style={{
            ...
            fontFamily: "monospace",
            color:
              delta < -0.05 ? "#22c55e" : delta > 0.05 ? "#ef4444" : "#64748b",
          }}
        >
          Δ vs TIA baseline: {delta > 0 ? "+" : ""}
          {(delta * 100).toFixed(0)}% v/c
          {delta < -0.05
            ? " ✓ improved"
            : delta > 0.05
              ? " ↑ worse"
...
function WhatIfGroup({ group, activeItems, onToggleItem }) {
  ...
  return (
    ...
    >
          ...
          <div style={{ textAlign: "left" }}>
            <div
              style={{
                ...
                fontWeight: 700,
                fontFamily: "'DM Mono', monospace",
                color: anyActive ? group.color : "#e2e8f0",
              }}
            >
              {group.label}
            </div>
            <div
              style={{ fontSize: 9, color: "#64748b", fontFamily: "monospace" }}
            >
              {group.description}
            </div>
          </div>
      ...
      {expanded && (
        <div
          style={{ borderTop: "1px solid #ffffff10", padding: "8px 12px 12px" }}
        >
              ...
              <div style={{ flex: 1 }}>
                <div
                  style={{
                    ...
                    fontWeight: 700,
                    fontFamily: "'DM Mono', monospace",
                    color: activeItems[item.id] ? group.color : "#94a3b8",
                    marginBottom: 2,
                  }}
                >
                  {item.label}
...
export default function App() {
  ...
  if (whatIfItems.public_transport) adjTrips *= 0.9;
  if (whatIfItems.lift_clubs) adjTrips *= 0.95;
  const tripsRemoved = Math.round(trips - adjTrips);

  const activeGroupCount = WHATIF_GROUPS.filter((g) =>
    g.items.some((i) => whatIfItems[i.id]),
  ).length;
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 10, color: "#475569", marginTop: 2 }}>
            AM Peak School Run · Based on TIA ITS 4839 (July 2025) · Erf 1061,
            Bergvliet
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {whatIfActive && (
            ...
            >
              ⚠ WHAT IF... ACTIVE ({activeGroupCount} group
      ...
      >
            ...
            >
              {[
                ...
                return (
                  ...
                  >
                    <div
                      style={{ fontSize: 9, color: "#64748b", marginBottom: 3 }}
                    >
                      {label}
                    </div>
                    <div
                    ...
                    {seg?.studied === false && (
                      <div
                        style={{ fontSize: 9, color: "#a855f7", marginTop: 2 }}
                      >
                        ⚠ UNSTUDIED
                      </div>
                    )}
        ...
        >
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px" }}>
            ...
            >
              {[
                ...
                {
                  label: "Peak Window",
                  value: `${SCENARIOS[scenario].peakMin} min`,
                  color: "#60a5fa",
                },
                {
                  label: "Adjusted Trips",
                  value: whatIfActive
                    ? Math.round(adjTrips).toLocaleString()
                    : "—",
                  color: "#a78bfa",
                },
                {
                  label: "Trips Removed",
                  value:
                    whatIfActive && tripsRemoved > 0 ? `−${tripsRemoved}` : "—",
                  color: "#22c55e",
                },
            ...
            >
              Toggle groups below to overlay variables the TIA did not model.
              Changes apply live on top of the active scenario.
            </div>
          ...
          >
            ...
            Based on ITS 4839 TIA Draft + ITS Global Independent Review Mar
            2026.
          </div>
        </div>
      </div>
